        ...
        "vehicles": [
            {
                ...
                "route": [[-13.1, -0.8], [-10.0, -0.2], [-7.0, -0.8]],
                "tick_owner": True,
            },
            {
                "vehicle_id": 2,
                "spawn_transform": {"x": -13.1, "y": -2.0, "z": 0.5, "yaw": 0.0},
                "route": [[-13.1, 2.0], [-10.0, 2.5], [-7.0, 2.0]],
            },
        ],
    }
...
class TestCarlaMultiProcess(unittest.TestCase):
    def test_two_vehicle_processes_record_independent_trajectories(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            setup_file.write_text(json.dumps(_manifest()), encoding="ascii")
            ...
            try:
                ...
                for vehicle_id in (1, 2):
                    ...
                    command = [
                        sys.executable, "-m", "extra.launch.carla_process_runner",
                        "--setup-file", str(setup_file), "--vehicle-id", str(vehicle_id),
    ...
    def _wait_for_files(self, paths, processes, timeout_s=15.0):
