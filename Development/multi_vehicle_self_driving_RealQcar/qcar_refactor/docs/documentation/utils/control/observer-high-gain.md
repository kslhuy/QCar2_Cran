# `utils/control/observer/observer_high_gain.py`

## 1. Introduction

This inactive historical module is byte-identical to `observer_ekf.py` and
exports `ObserverEKF`, not a high-gain observer class. Current configuration
marks the `high_gain` profile unsupported.
...
| `ObserverEKF(config, vehicle_id=0, logger=None, wheelbase=None)` | Profile and optional wheelbase override | EKF observer | This historical high-gain module actually exports `ObserverEKF`; it contains the copied EKF implementation. |
...
to `VehicleStateEstimate`; no separate high-gain algorithm is implemented.

## 4. Position in the project

[[module-factory|module_factory]] does not import this file and
`config_observer.yaml` explicitly rejects the high-gain profile. It has no
