...
from ..ground_station_type import GroundStationCommandRequest
...
class GroundStationCommandHandler:
    ...
    def parse(self, text: str) -> GroundStationCommandRequest:
        ...
        if action == "lidar":
            if len(parts) not in {2, 3} or parts[1].lower() not in {"start", "stop", "status"}:
                raise ValueError("lidar requires: lidar <start|stop|status> [vehicle-id]")
            operation = parts[1].lower()
            if operation in {"start", "stop"} and len(parts) != 3:
                raise ValueError(f"lidar {operation} requires a vehicle ID")
            if len(parts) == 2:
                return GroundStationCommandRequest("lidar-status")
            vehicle_id = self._vehicle_id(parts[2])
            if operation == "status":
                return GroundStationCommandRequest("lidar-status", vehicle_id=vehicle_id)
            command_type = (
                CommandType.ENABLE_LIDAR_DIAGNOSTIC
                if operation == "start"
                else CommandType.DISABLE_LIDAR_DIAGNOSTIC
            )
            return GroundStationCommandRequest(
                f"lidar-{operation}",
                vehicle_id=vehicle_id,
                command=VehicleCommand(command_type, source=CommandSource.LOCAL),
            )
        if action == "status":
            if len(parts) != 2:
