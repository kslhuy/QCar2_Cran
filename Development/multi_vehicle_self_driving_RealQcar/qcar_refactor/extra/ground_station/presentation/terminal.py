...
from ..core.command_handler import GroundStationCommandHandler
...
from ..core.server import GroundStationServer
from ..utils.live_plotting import LiveLidarViewer, LivePlottingError


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the operator-side ground-station terminal")
    ...
    parser.add_argument("--refresh-hz", type=float, help="dashboard refresh rate")
    parser.add_argument(
        "--lidar-refresh-hz",
        type=float,
        help="Qt LiDAR viewer refresh rate, independent of the terminal dashboard",
    )
    parser.add_argument("--stale-after-s", type=float, help="mark snapshots stale after this age")
    ...
    parser.add_argument("--ack-timeout-s", type=float, help="operator wait time for COMMAND_ACK")
    ...
    try:
        configuration = load_ground_station_configuration().with_overrides(
            ...
            lidar_refresh_hz=args.lidar_refresh_hz,
            stale_after_s=args.stale_after_s,
            acknowledgement_timeout_s=args.ack_timeout_s,
            dashboard_enabled=False if args.no_dashboard else None,
            input_enabled=False if args.no_input else None,
        )
    ...
    dashboard = GroundStationDashboard(stale_after_s=configuration.stale_after_s)
    command_handler = GroundStationCommandHandler()
    terminal_ui_enabled = configuration.dashboard_enabled and sys.stdout.isatty()
    if terminal_ui_enabled:
        terminal = GroundStationTerminal(
            server,
            dashboard,
            command_handler,
            listen_host=configuration.listener_host,
            refresh_hz=configuration.dashboard_refresh_hz,
            lidar_refresh_hz=configuration.lidar_refresh_hz,
            duration_s=args.duration_s,
            acknowledgement_timeout_s=configuration.acknowledgement_timeout_s,
            input_enabled=configuration.input_enabled,
        )
        try:
            server.start()
            return terminal.run()
    ...
    if configuration.input_enabled:
        input_thread = threading.Thread(
            ...
            args=(server, command_handler, stop_requested, configuration.acknowledgement_timeout_s),
            name="GroundStationOperatorInput",
            daemon=True,
        )
    try:
        server.start()
        print(f"Ground station listening on {configuration.listener_host}:{server.port}")
        if input_thread is not None:
            input_thread.start()
        started_at = time.monotonic()
        refresh_period_s = 1.0 / configuration.dashboard_refresh_hz
...
class GroundStationTerminal:
    ...
    def __init__(
        ...
        *,
        ...
        self._lidar_refresh_period_s = 1.0 / lidar_refresh_hz
        ...
        self._lidar_viewers: dict[int, LiveLidarViewer] = {}
        self._lidar_requested: set[int] = set()
        self._lidar_view_lock = threading.Lock()
    ...
    def run(self) -> int:
        ...
        self._record("LiDAR display control is ready: lidar start|stop <vehicle-id> | lidar status [vehicle-id]")
        try:
            self._app.run(pre_run=self._start_background_tasks)
        except KeyboardInterrupt:
            pass
        finally:
            self._close_lidar_viewers(send_stop=True)
        return 0

    def _start_background_tasks(self) -> None:
        self._app.create_background_task(self._refresh_loop())
        self._app.create_background_task(self._lidar_view_loop())
    ...
    async def _lidar_view_loop(self) -> None:
        """Refresh Qt plots at their own rate without delaying the terminal UI."""

        while not self._stop_requested:
            self._refresh_lidar_viewers()
            await asyncio.sleep(self._lidar_refresh_period_s)
    ...
    def _execute_command(self, text: str) -> None:
        ...
        if request.action == "help":
            ...
            self._record("enable-manual <id> | disable-manual <id> | manual-drive <id> | lidar start|stop <id> | lidar status [id] | list | status <id> | quit")
            return
        if request.action == "lidar-status":
            self._report_lidar_status(request.vehicle_id)
            return
        ...
        if result.get("outcome") != "applied":
            return
        if request.action == "lidar-start":
            with self._lidar_view_lock:
                self._lidar_requested.add(request.vehicle_id)
            self._record(f"LiDAR display requested for vehicle {request.vehicle_id}; waiting for its first TCP scan")
        elif request.action == "lidar-stop":
            with self._lidar_view_lock:
                self._lidar_requested.discard(request.vehicle_id)
    ...
    def _report_lidar_status(self, vehicle_id: int | None) -> None:
        vehicle_ids = (
            (vehicle_id,)
            if vehicle_id is not None
            else tuple(row["vehicle_id"] for row in self._server.session_rows())
        )
        if not vehicle_ids:
            self._record("No registered vehicles have LiDAR status")
            return
        for current_vehicle_id in vehicle_ids:
            scan = self._server.latest_lidar_scan(current_vehicle_id)
            with self._lidar_view_lock:
                viewer_state = "open" if current_vehicle_id in self._lidar_viewers else (
                    "awaiting scan" if current_vehicle_id in self._lidar_requested else "off"
                )
            scan_state = "received" if scan is not None else "none"
            self._record(f"LiDAR vehicle {current_vehicle_id}: {viewer_state}; latest TCP scan: {scan_state}")

    def _refresh_lidar_viewers(self) -> None:
        with self._lidar_view_lock:
            requested = tuple(self._lidar_requested)
        for vehicle_id in requested:
            scan = self._server.latest_lidar_scan(vehicle_id)
            viewer = self._lidar_viewers.get(vehicle_id)
            if scan is not None and viewer is None:
                try:
                    viewer = LiveLidarViewer(title=f"Vehicle {vehicle_id} LiDAR - ground station TCP")
                    self._lidar_viewers[vehicle_id] = viewer
                    self._record(f"LiDAR viewer opened for vehicle {vehicle_id}")
                except LivePlottingError as error:
                    self._record(f"LiDAR viewer failed for vehicle {vehicle_id}: {error}")
                    with self._lidar_view_lock:
                        self._lidar_requested.discard(vehicle_id)
                    self._server.send_command(vehicle_id, VehicleCommand(CommandType.DISABLE_LIDAR_DIAGNOSTIC))
                    continue
            if viewer is not None and scan is not None:
                viewer.show_scan(scan, vehicle_id=vehicle_id)
        for vehicle_id, viewer in tuple(self._lidar_viewers.items()):
            with self._lidar_view_lock:
                requested = vehicle_id in self._lidar_requested
            if not requested:
                viewer.close()
                del self._lidar_viewers[vehicle_id]
                continue
            if viewer.process_events():
                continue
            del self._lidar_viewers[vehicle_id]
            with self._lidar_view_lock:
                self._lidar_requested.discard(vehicle_id)
            self._record(f"LiDAR viewer closed for vehicle {vehicle_id}; disabling its vehicle-side diagnostic")
            self._server.send_command(vehicle_id, VehicleCommand(CommandType.DISABLE_LIDAR_DIAGNOSTIC))

    def _close_lidar_viewers(self, *, send_stop: bool) -> None:
        for viewer in self._lidar_viewers.values():
            viewer.close()
        self._lidar_viewers.clear()
        with self._lidar_view_lock:
            requested = tuple(self._lidar_requested)
            self._lidar_requested.clear()
        if send_stop:
            for vehicle_id in requested:
                self._server.send_command(vehicle_id, VehicleCommand(CommandType.DISABLE_LIDAR_DIAGNOSTIC))
...
def _operator_input_loop(
    ...
    while not stop_requested.is_set():
        ...
        if request.action == "help":
            ...
            print("     lidar start|stop <vehicle-id> | lidar status [vehicle-id]")
            continue
        if request.action == "lidar-status":
            vehicle_ids = (
                (request.vehicle_id,)
                if request.vehicle_id is not None
                else tuple(row["vehicle_id"] for row in server.session_rows())
            )
            for vehicle_id in vehicle_ids:
                print(
                    f"LiDAR vehicle {vehicle_id}: "
                    f"{'latest TCP scan received' if server.latest_lidar_scan(vehicle_id) is not None else 'no TCP scan'}"
                )
            continue
        if request.action == "list":
