...
class QCarInitializer:
    ...
    def InitEnv(self):
        ...
        camera.spawn_degrees(
            location=[-0.812, -1.651, 1.233],
            rotation=[0, 37.328, 75.423]
        )
        camera.possess()
...
def spawn_environment_objects(qlabs_instance):
    ...
    environment_objects["crosswalks"][0].spawn(
        location=[-5, 9.5, 0],
    ...
    environment_objects["crosswalks"][1].spawn(
        location=[1.3, 16, 0], rotation=[0, 0, 0], scale=[1, 1, 0.75], configuration=0
    )

    environment_objects["crosswalks"][2].spawn(
        location=[7.7, 9.5, 0],
        rotation=[0, 0, np.pi / 2],
        scale=[1, 1, 0.75],
        configuration=0,
    ...
    environment_objects["crosswalks"][3].spawn(
        location=[1.3, 3, 0], rotation=[0, 0, 0], scale=[1, 1, 0.75], configuration=0
    )
    ...
    environment_objects["traffic_lights"][0].spawn(
        location=[-3.77, 13, 0], rotation=[0, 0, np.pi / 2], configuration=0
    )

    environment_objects["traffic_lights"][1].spawn(
        location=[4.9, 14.8, 0], rotation=[0, 0, 0], configuration=0
    )

    environment_objects["traffic_lights"][2].spawn(
        location=[6.7, 5.7, 0], rotation=[0, 0, -np.pi / 2], configuration=0
    )

    environment_objects["traffic_lights"][3].spawn(
        location=[-2, 4.27, 0], rotation=[0, 0, np.pi], configuration=0
    )
    ...
    yieldSign.spawn(location=[0.4, -13, 0], rotation=[0, 0, np.pi])
    ...
    for i in range(NUMROUNDABOUTSIGNS):
        environment_objects["roundabout_signs"].append(
            QLabsRoundaboutSign(qlabs_instance)
        )

    environment_objects["roundabout_signs"][0].spawn(
        location=[24.5, 33, 0], rotation=[0, 0, -np.pi / 2]
    )

    environment_objects["roundabout_signs"][1].spawn(
        location=[4.5, 40, 0], rotation=[0, 0, np.pi]
    )

    environment_objects["roundabout_signs"][2].spawn(
        location=[10.6, 28.5, 0], rotation=[0, 0, np.pi]
    )
...
def load_fleet_config(
    path: str = "./longitudinal_config.yaml",
) -> dict:
...
def build_qcars_from_config(
    ...
    for v in vehicles_to_use:
        ...
        print(
            f"[spawn] vehicle {v.get('car_id')} assigned to path {path_number} starting at node {first_node_id}"
        )
        ...
        if node_entry and "pose" in node_entry:
            ...
            rotation = [0, 0, pose[2]]
            print(
                f"[spawn] vehicle {v.get('car_id')} uses node {first_node_id} pose from fleet_config: {pose}"
            )
        else:
            ...
            if pose:
                ...
                rotation = [0, 0, pose[2]]
                print(
                    f"[spawn] vehicle {v.get('car_id')} uses fallback node {first_node_id} pose: {pose}"
                )
            else:
                # absolute fallback
                ...
                rotation = [0, 0, 0.0]
                print(
                    f"[spawn] vehicle {v.get('car_id')} has no node pose available, using {location} {rotation}"
                )
...
def get_transform_input(car_number):
    ...
    if cmd == "E":
        ...
        if qlabs_temp.open("localhost"):
            env_action = (
                input("  [S]pawn or [D]estroy environment objects? ").strip().upper()
            )
            if env_action == "S":
