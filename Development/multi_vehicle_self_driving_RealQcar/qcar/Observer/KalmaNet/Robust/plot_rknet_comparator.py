...
def main() -> None:
    ...
    ax = axes[3, 0]
    ax.plot(time_axis, data["K_x_x"], label="K(x,x) -> Gain for GPS X", linewidth=1.2)
    ax.plot(time_axis, data["K_y_y"], label="K(y,y) -> Gain for GPS Y", linewidth=1.2)
    ax.plot(time_axis, data["K_psi_psi"], label="K(psi,psi) -> Gain for IMU Heading", linewidth=1.2)
    ax.plot(time_axis, data["K_v_v"], label="K(v,v) -> Gain for Motor V", linewidth=1.2)
    ax.set_title("Kalman Gain (K) Diagonals")
    ax.set_xlabel("time [s]")
    ax.grid(True, alpha=0.3)
    ax.legend()

    # Move Mask Decision Summary to 3, 1
    ax = axes[3, 1]
