...
Before evaluating any continuous trust score, the host vehicle $i$ first applies a binary validity test to each packet received from vehicle $l$. This preliminary gate is used to reject clearly unusable data before entering the physical-consistency checks. In particular, a packet is considered valid only if it passes the communication-layer security and timing requirements, namely message authentication and freshness:
\begin{equation}
    \delta_{i,l}(t)=
    \begin{cases}
    ...
    \end{cases}
\end{equation}
The authentication condition ensures that the packet originates from the claimed sender and has not been altered during transmission, while the freshness condition prevents delayed or replayed packets from being treated as current information. Hence, $\delta_{i,l}(t)$ acts as a hard communication-quality gate: when $\delta_{i,l}(t)=0$, the received data are treated as unavailable and are excluded from the instantaneous trust update.

This binary indicator is intentionally separated from the trust scores $\LT_{i,l}$ and $\DT_{i,l}$. The role of $\delta_{i,l}(t)$ is not to quantify how reliable the content is, but to decide whether the content is admissible at all. Once a packet passes this admissibility test, the trust layer evaluates whether its \emph{content} is physically and statistically consistent with the host vehicle's observations and distributed estimates.
...
For each host vehicle $i$ and target vehicle $j$, the implementation uses a target-wise correction step followed by prediction. The correction stage can be written as
\begin{equation}\label{eq:code_correction}
\hat{x}_{i,c}^{(j)}(t)=
w_{ii}^{(j)}(t)\hat{x}_{i}^{(j)}(t)
+w_{i0}^{(j)}(t)\hat{x}_{0}^{(j)}(t)
+\sum_{l\in \mathcal{N}_{i}^{(j)}(t)} w_{il}^{(j)}(t)\hat{x}_{l}^{(j)}(t),
\end{equation}
and the prediction stage is
\begin{equation}\label{eq:code_prediction}
\hat{x}_{i}^{(j)}(t+1)=A_{\text{b}}\hat{x}_{i,c}^{(j)}(t)+B_{\text{b}}\hat{u}_{j}(t).
\end{equation}
The residual self-weight is defined by
\begin{equation}\label{eq:self_weight}
w_{ii}^{(j)}(t)=1-w_{i0}^{(j)}(t)-\sum_{l\in\mathcal{N}_{i}^{(j)}(t)}w_{il}^{(j)}(t),
\end{equation}
where $\mathcal{N}_{i}^{(j)}(t)$ denotes the neighbors of host $i$ that are trusted at time $t$ and currently provide an estimate of target $j$.

To match the code, the direct, self, and neighbor channels are first assigned raw scores. Let $\bar w_0>0$ and $\bar w_{\rm self}>0$ be fixed gains, and define the neighbor budget $\bar w_{\rm N}=1-\bar w_0-\bar w_{\rm self}$. Then
\begin{subequations}\label{eq:raw_weights}
\begin{align}
\tilde{w}_{i0}^{(j)}(t) &= \bar w_0\,\LT_{i,j}(t)\,\phi_{i0}^{(j)}(t),\\
\tilde{w}_{ii}^{(j)}(t) &= \bar w_{\rm self}\,\phi_{ii}^{(j)}(t),\\
\tilde{w}_{il}^{(j)}(t) &= \bar w_{\rm N}\,\phi_{i,\mathrm{N}}^{(j)}(t)\,
\frac{\O_i(l,t)}{\sum\limits_{p\in\mathcal{N}_{i}^{(j)}(t)}\O_i(p,t)},
\qquad l\in\mathcal{N}_{i}^{(j)}(t).
\end{align}
\end{subequations}
The final normalized gains are
\begin{equation}\label{design_gain}
w_{iq}^{(j)}(t)=
\frac{\tilde{w}_{iq}^{(j)}(t)}
{\tilde{w}_{i0}^{(j)}(t)+\tilde{w}_{ii}^{(j)}(t)+\sum_{l\in\mathcal{N}_{i}^{(j)}(t)}\tilde{w}_{il}^{(j)}(t)},
\end{equation}
for every active channel $q\in\{0,i\}\cup\mathcal{N}_{i}^{(j)}(t)$.

The multipliers $\phi_{i0}^{(j)}(t)$, $\phi_{ii}^{(j)}(t)$, and $\phi_{i,\mathrm{N}}^{(j)}(t)$ encode the trust flags produced by the estimator:
\begin{itemize}
    \item under nominal conditions, all three multipliers are equal to $1$;
    \item when the target-$j$ direct/local channel becomes suspicious, the direct term is reduced and the residual confidence is shifted toward the self-prediction channel;
    \item when the fleet-consensus channel is suspicious but the direct target packet remains reliable, the direct channel is reinforced and the neighbor channels are reduced.
\end{itemize}
In addition, each neighbor contribution is upper bounded by a design cap $\bar w_{\max}$, and any overflow is reassigned to the self channel. Hence,
\begin{equation}\label{eq:weight_simplex}
w_{ii}^{(j)}(t)\ge 0,\qquad
w_{i0}^{(j)}(t)\ge 0,\qquad
w_{il}^{(j)}(t)\ge 0,\qquad
w_{ii}^{(j)}(t)+w_{i0}^{(j)}(t)+\sum_{l\in\mathcal{N}_{i}^{(j)}(t)}w_{il}^{(j)}(t)=1.
\end{equation}

\paragraph{Presentation summary.}
The meaning of the three channels is:
\begin{itemize}
    \item $w_{i0}^{(j)}$: trust in the direct packet or anchor for target $j$;
    \item $w_{il}^{(j)}$: trust in neighbor $l$ when estimating target $j$;
    \item $w_{ii}^{(j)}$: confidence kept on the host's own prediction.
\end{itemize}
...
\subsection{Contamination rollback mechanism}\label{subsec:rollback}
The trust-adaptive weights protect future updates, but in practice a malicious packet can already contaminate the estimate before the trust score drops. For this reason, the implementation stores a short replay buffer for each target vehicle.

At each time $k$, host vehicle $i$ stores the pre-update estimate and the information used in the correction/prediction steps:
\begin{equation}\label{eq:rollback_buffer}
\mathcal{B}_{i}^{(j)}(k)=
\Big\{
\hat{x}_{i}^{(j)}(k^{-}),
\hat{x}_{0}^{(j)}(k),
\{\hat{x}_{l}^{(j)}(k)\}_{l\in\mathcal{N}_{i}^{(j)}(k)},
\Delta t_k,
\hat{u}_j(k)
\Big\},
\end{equation}
together with the applied weights $w_{i0}^{(j)}(k)$ and $w_{il}^{(j)}(k)$.

If target $j$ becomes suspicious at time $t$ because $\T_{i,j}(t)<\theta_{\min}$ or a consistency flag is raised, the host constructs a malicious set $\mathcal{M}_{i}(t)$ and restarts from the latest trusted snapshot
\begin{equation}
\hat{x}_{i,\mathrm{tr}}^{(j)}(t_{\mathrm{r}}), \qquad t_{\mathrm{r}}\le t.
\end{equation}
The buffered window is then replayed with the same observer law, but excluding all malicious contributors:
\begin{equation}\label{eq:rollback_correction}
\hat{x}_{i,c}^{(j)}(k)=
\hat{x}_{i}^{(j)}(k^{-})
+\mathbf{1}_{\{j\notin\mathcal{M}_{i}(t)\}}\,
w_{i0}^{(j)}(k)\big(\hat{x}_{0}^{(j)}(k)-\hat{x}_{i}^{(j)}(k^{-})\big)
+\sum_{l\in\mathcal{N}_{i}^{(j)}(k)\setminus\mathcal{M}_{i}(t)}
w_{il}^{(j)}(k)\big(\hat{x}_{l}^{(j)}(k)-\hat{x}_{i}^{(j)}(k^{-})\big),
\end{equation}
\begin{equation}\label{eq:rollback_prediction}
\hat{x}_{i}^{(j)}(k+1)=A_{\text{b}}\hat{x}_{i,c}^{(j)}(k)+B_{\text{b}}\hat{u}_j(k),
\qquad k=t_{\mathrm{r}},\ldots,t-1.
\end{equation}
Therefore, rollback is not a different observer. It is the same observer replayed over a short window after removing corrupted channels.

\paragraph{Presentation summary.}
The rollback logic can be stated in three steps:
\begin{itemize}
    \item record recent correction terms and prediction inputs;
    \item when trust drops, jump back to the last trusted state;
    \item replay the buffered window without malicious contributors.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Hybrid-system analysis for rollback.}
    The current implementation already includes a rollback buffer and replay mechanism to remove contaminated updates after trust-triggered detection. However, the associated theory remains open: once rollback is activated, the observer becomes a hybrid system with state resets and short replay intervals. A useful future direction is to derive sufficient conditions ensuring that the reset-and-replay map remains bounded, does not amplify disturbances, and preserves the ISS-type convergence properties of the nominal observer under repeated rollback events.
