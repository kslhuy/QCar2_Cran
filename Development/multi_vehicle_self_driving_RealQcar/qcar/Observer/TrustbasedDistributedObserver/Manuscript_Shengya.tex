...
\begin{itemize}
    ...
    \item A trust-adaptive observer-weight design with admissible mixing coefficients and direct-anchor checkable, sparse-graph monitorable local nonlinear input-to-state stability (ISS) conditions for bounded modeling, input, attack, and nonlinear mismatch terms in the bicycle-error dynamics.
...
Formal secure/resilient estimation~\cite{fawzi2014secure,mitra2018byzantine,khazraei2020attack_auth} & Sparse, Byzantine, stealthy attacks & Partial & No & Worst-case & Yes & No & No \\
...
Consensus and distributed-estimation methods exploit redundancy across multiple agents or sensors~\cite{Guitao_multi_mesure,Consensus_discret_Saber,Plug_play}. They can improve robustness by cross-validating information, but many approaches implicitly require sufficiently reliable neighbors or fixed communication weights. Under coordinated attacks, persistent packet loss, or biased local observers, fixed consensus weights can spread wrong estimates instead of isolating them. Formal secure and Byzantine-resilient estimation provides worst-case guarantees under sparse attacked sensors or bounded adversary fractions~\cite{fawzi2014secure,mitra2018byzantine,khazraei2020attack_auth}. These results are stronger than the present method on adversary-count guarantees and stealthy worst-case analysis. In contrast, the present objective is to adapt V2V observer weights online for a nonlinear platoon estimator and to connect the estimate to rollback and closed-loop use. More general secure-estimation methods also include bank-of-observers selection under sparse sensor attacks~\cite{chong2021secure} and distributed $\ell_1$ state-and-fault estimation for multi-agent systems~\cite{hashimoto2020distributed_l1}; these provide useful resilient-estimation baselines, but they do not directly provide trust-adaptive V2V observer weights for platoons.
...
\end{aligned}
\end{equation}
Because this sum mixes physical units, it is used only as a relative severity index within the same setup. A unit-consistent scaled metric, matching the contraction norm used in Section~\ref{sec_weight_design}, is
\begin{equation}
\operatorname{RMSE}_{*,i}=
\sqrt{\frac{1}{|\mathcal{T}|}\sum_{t\in\mathcal{T}}
\left\|D_x\big(x_i(t)-\hat x_i(t)\big)\right\|_\infty^2}.
\label{eq_scaled_rmse_metric}
\end{equation}
False-positive trust rejection over a benign validation set $\mathcal{B}$ is
\begin{equation}
\mathrm{FPR}_{\mathrm{trust}}=
\frac{1}{|\mathcal{B}|}
\sum_{(i,l,t)\in\mathcal{B}}
\mathbf{1}\{\VT_{i,l}(t)<\theta_{\min}\}.
\label{eq_false_positive_metric}
\end{equation}
The saved data do not contain the full synchronized state/input histories needed to recompute~\eqref{eq_scaled_rmse_metric}, post-attack estimation-error recovery, or a benign noise/jitter sweep for~\eqref{eq_false_positive_metric}. Therefore the current numerical tables retain the raw severity index, while regenerated campaign logs should report~\eqref{eq_scaled_rmse_metric} and~\eqref{eq_false_positive_metric} as first-class metrics. The closed-loop check is limited to positive path-projected spacing during the attack interval.
...
The ISS result also depends on bounded disturbances, bounded input mismatch, and sufficient trusted connectivity over time. In particular, the receiving-side projection~\eqref{eq_received_packet_gate} converts accepted attacks into bounded inputs; the bound then appears through $\bar d_{\mathrm{att}}$ in Theorem~\ref{thm_observer}. This is a sufficient-condition robustness statement, not a worst-case stealth-security guarantee. A slowly ramping, physically consistent bias that remains inside $\Omega_z$ and just below the trust threshold can still contribute a nonzero steady-state term before it is rejected. The natural stress test is therefore a near-threshold ramp experiment that reports the induced $\bar d_{\mathrm{att}}$, the scaled steady-state offset~\eqref{eq_scaled_rmse_metric}, and the false-positive/recovery tradeoff as $\theta_{\min}$ and $\beta_{\text{trust}}$ vary.

If an attack removes all reliable paths to a target vehicle, no distributed observer can recover that target state from communication alone. Similarly, the rollback module improves delayed-decision recovery only when the required packets and weights remain inside the finite buffer and when trust-triggered resets do not occur arbitrarily fast. The current saved simulation campaign does not include benign noise/jitter sweeps or synchronized post-attack state histories, so false-positive rates and estimation-error recovery should be regenerated rather than inferred from the attack-only tables. The current closed-loop evidence is limited to projected spacing preservation during the attack interval; string stability, sparse-topology operation, coordinated attacks, Monte Carlo noise/dropout behavior, near-threshold stealthy-bias attacks, and sensitivity to trust parameters require deeper validation.
...
Future work will focus on a deeper string-stability study of the closed loop, regenerated Monte Carlo validation with scaled-norm RMSE and benign false-positive sweeps, near-threshold slowly ramping bias attacks, matrix-valued trust weights for channel-wise attack isolation, tighter dwell-time and contraction conditions for rollback-triggered resets, and stronger coupling between trust and control under dynamic communication topologies. Extending the QCar/LIMO validation to larger fleets and richer vehicle models is also a natural next step.
