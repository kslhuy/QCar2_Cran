...
Motivated by the residual deviation observed in the CACC-only platform rollback test, this subsection formulates a mixed local/cooperative longitudinal controller. The purpose is to combine the cooperative following benefit of CACC with a local ACC fallback when the distributed estimates become unreliable. The bicycle-state observer remains in planar coordinates, and the spacing controller uses the road-aligned projection
\begin{align}
    e_{\parallel}(k)&=[\cos\psi_{\mathrm{ref}}(k),\sin\psi_{\mathrm{ref}}(k)]^\top,\\
    s_i(k)&=p_i(k)^\top e_{\parallel}(k),
\end{align}
where $p_i=[X_i,Y_i]^\top$. For each follower $i\ge2$, the controller computes two commands: a local ACC command driven by the measured or synthesized direct-leader gap and a cooperative CACC command driven by the distributed observer estimates of the predecessors $\mathcal{P}_i=\{1,\ldots,i-1\}$. The final longitudinal command is obtained by trust-based command fusion, so unreliable V2V estimates affect only the cooperative channel.
...
The local ACC branch uses the measured or trusted synthesized gap to the immediate predecessor. Let $s_{i-1,i}$ denote this projected gap and define the constant-time-headway local ACC target
\begin{equation}
    s_{\mathrm{ACC},i}^{\star}(k)
    =d_{\mathrm{ACC}}+h_{\mathrm{ACC}}\max\{\hat{v}_0^{(i)}(k),0\}.
\end{equation}
The local gap error is
\begin{equation}
    e_{\mathrm{ACC},i}(k)=s_{i-1,i}(k)-s_{\mathrm{ACC},i}^{\star}(k).
\end{equation}
The ACC branch maps this error into a local target speed
\begin{equation}
v_{\mathrm{ACC},i}^{\star}(k)=
\begin{cases}
0, & s_{i-1,i}(k)\le s_{\mathrm{stop}},\\
\mathrm{sat}_{[v_{\min},v_{\max}]}
\left(v_{\mathrm{base},i}(k)+k_d e_{\mathrm{ACC},i}(k)\right),
& s_{i-1,i}(k)>s_{\mathrm{stop}},
\end{cases}
\end{equation}
and the low-level speed loop converts $v_{\mathrm{ACC},i}^{\star}$ into the local command $u_{\mathrm{loc},i}$. The parameters $d_{\mathrm{ACC}}$, $h_{\mathrm{ACC}}$, $k_d$, $s_{\mathrm{stop}}$, $v_{\min}$, and $v_{\max}$ are the stand-off distance, time headway, gap-error speed gain, stop distance, and speed bounds. Since $u_{\mathrm{loc},i}$ does not depend on distributed V2V estimates, it provides the fallback when the cooperative layer is down-weighted.
...
The final target command blends the local and cooperative commands as
\begin{equation}
    u_{\text{target},i}=(1-\gamma_i(t))u_{\mathrm{loc},i}+\gamma_i(t)u_{\mathrm{coop},i},
\end{equation}
...
\begin{equation}
    u_i(t)=u_i(t-1)+\tau_f\big(u_{\text{target},i}-u_i(t-1)\big),
\end{equation}
