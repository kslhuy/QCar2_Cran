...
from Observer.TrustbasedDistributedObserver.trust_model import (
    TriPTrustModel, TrustConfig, TrustScore, VehicleData
)
from Observer.TrustbasedDistributedObserver.weight_trust_module import (
    WeightTrustModule, WeightConfig, WeightResult
)
...
class TrustBasedFleetEstimator(FleetStateEstimatorBase):
    ...
    def __init__(self, vehicle_id: int, fleet_size: int, state_dim: int = 5,
        ...
        self.trust_config = TrustConfig(
            weight_velocity=trust_config_dict.get('weight_velocity', 12.0),
    ...
    def _trust_weighted_update(self, target_id: int, current_time_ns: int,
    ...
    def _predict_dynamics(self, state: np.ndarray, control: np.ndarray, 
                          dt: float) -> np.ndarray:
        """
    ...
    def _apply_attack_mitigation(self, trust_scores: Dict[int, float],
                                  current_time_ns: int):
        """
...
def create_trust_based_estimator(estimator_type: str, vehicle_id: int, 
                                  fleet_size: int, state_dim: int = 5,
                                  config: Dict = None, logger=None):
    """
