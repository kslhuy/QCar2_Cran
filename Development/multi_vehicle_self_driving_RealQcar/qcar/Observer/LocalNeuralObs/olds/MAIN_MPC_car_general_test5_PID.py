...
Type_simulation = 'train'  # or 'evaluation' 
...
Type_learning = Type_learning_map.get(3)
size_dict = 20
...
batch_size = 3  # Number of ccummulation the loss ,  Number of samples before each training step 
...
if (LossType == 'refs'):
    ...
    Weight_v_x, Weight_v_y , Weight_psi , Weight_psi_dot   = 10, 20000, 10, 20000
    # Weight_v_x, Weight_v_y , Weight_psi , Weight_psi_dot   = 1, 2, 1, 2
...
for epoch in range(max_epochs):
    ...
    f_uk = np.zeros((D_out, 1))
    f_uk_Save = np.zeros((D_out, len(t)))
    ...
    # Todo Save this dx_df to see more 
    ...
    for i in range(0,sim_length-1):
        ...
        states_obs_new , f_hat_new=  support.observer_with_D(states_obs,U1,U2 , measurements, f_hat , f_nn  )

        states_obs2_new , f_hat2_new=  support.observer2_with_D(states_obs2,U1,U2 , measurements, f_hat2 ,  0 )
        ...
        dx_df  = carGrad.GradientSolver_discret_general_w_D(dx_df) # expected shape (4 , D_out ) = nominateur , denominateur
        ...
        # statesTotal_obs_k = statesTotal_obs[i+1][0:len(states_obs)]

        statesTotal_obs_k = states_obs
        ...
        if (Type_learning == 'learningby_Dict'):
            ...
            dldf, loss_track = chainRule_func(input_target, statesTotal_obs_k, dx_df, f_uk_Save[:,i], f_nn, weight_error_traj, lamda)
        ...
        # print(f"Time between iteration {i}: {time_diff} seconds")
    ...
    scipy.io.savemat(os.path.join(folder_save_matlab_data,f'test3_training_data_{epoch}.mat'), {
