...
class KeyboardController:
    ...
    def get_control_state(self) -> ManualControlState:
        ...
        # Gradual steering (smoother decay to prevent stuttering)
        if cfg.left_key in keys:
            self._current_steering = min(
                self._current_steering + cfg.steering_scale * 0.2, cfg.steering_scale
            )
            ...
            # Decay steering when no input (smoother decay: 0.9 instead of 0.7)
