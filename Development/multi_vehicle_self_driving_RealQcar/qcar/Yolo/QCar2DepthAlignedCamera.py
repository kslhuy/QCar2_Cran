...
class QCar2DepthAlignedCamera:
    ...
    def __init__(self, 
        ...
        print(f"✓ QCar2DepthAlignedCamera initialized")
    ...
    def _calculate_focal_lengths(self):
        ...
        fx_rgb = (self.imageWidth / 2.0) / np.tan(self.fov_rgb_h / 2.0) * self.focal_scale_rgb
        fy_rgb = (self.imageHeight / 2.0) / np.tan(self.fov_rgb_v / 2.0) * self.focal_scale_rgb
        fx_depth = (self.imageWidth / 2.0) / np.tan(self.fov_depth_h / 2.0) * self.focal_scale_depth
        fy_depth = (self.imageHeight / 2.0) / np.tan(self.fov_depth_v / 2.0) * self.focal_scale_depth
    ...
    def _initialize_camera(self, video3dPort):
        ...
        self.camera = QCarRealSense(
            mode='RGB, Depth',
            focalLengthRGB=np.array([[self.fx_rgb/2], [self.fy_rgb/2]], dtype=np.float64),
            principlePointRGB=np.array([[self.cx_rgb], [self.cy_rgb]], dtype=np.float64),
            focalLengthDepth=np.array([[self.fx_depth/2], [self.fy_depth/2]], dtype=np.float64),
            principlePointDepth=np.array([[self.cx_depth], [self.cy_depth]], dtype=np.float64),
            skewDepth=0.0,
            skewRGB=0.0,
        ...
        try:
            ...
            # Get extrinsics for transformation
            ...
            print(f"  Using calculated intrinsics from FOV")
            self.depth_to_rgb_transform = np.eye(4)
    ...
    def _align_depth_to_rgb_intrinsics_fast(self, depth_image):
        ...
        valid_proj = (u_rgb >= 0) & (u_rgb < w_rgb) & (v_rgb >= 0) & (v_rgb < h_rgb) & (z_proj > 0)
        ...
        if 0 < hole_ratio < 0.5:  # Only inpaint if reasonable number of holes
            ...
            depth_inpainted = cv2.inpaint(depth_uint16, mask, inpaintRadius=2, flags=cv2.INPAINT_TELEA)
    ...
    def _align_depth_to_rgb_intrinsics_legacy(self, depth_image):
        ...
        if np.sum(mask) > 0 and np.sum(mask) < mask.size:
            depth_uint16 = (aligned_depth * 1000).astype(np.uint16)
            depth_inpainted = cv2.inpaint(depth_uint16, mask, inpaintRadius=3, flags=cv2.INPAINT_NS)
            aligned_depth = depth_inpainted.astype(np.float32) / 1000.0
    ...
    def load_alignment_settings(self):
        ...
        filename = "alignment_settings_intrinsics.json" if self.use_intrinsics else "alignment_settings_simple.json"
        ...
        try:
            ...
            if self.use_intrinsics and mode_name == "intrinsics":
                ...
                self.principal_point_offset_x = settings.get("principal_point_offset_x", 0.0)
                self.principal_point_offset_y = settings.get("principal_point_offset_y", 0.0)
                ...
                print(f"  Settings mode mismatch: expected {'intrinsics' if self.use_intrinsics else 'simple'}, got {mode_name}")
                return False
