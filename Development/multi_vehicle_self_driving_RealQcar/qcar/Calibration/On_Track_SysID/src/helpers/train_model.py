...
def get_model_param(racecar_version, package_path=None):
    ...
    yaml_file = os.path.join(package_root, 'models', racecar_version, racecar_version + '_pacejka.txt')
    with open(yaml_file, 'r') as file:
...
def nn_train(
    ...
    for i in range(1, num_of_iterations+1):
        plot_this_iteration = plot_model or (always_plot_last_iteration and i == num_of_iterations)
        ...
        for epoch in range(1, num_of_epochs+1):
            ...
            if (epoch == num_of_epochs):
                pbar.close()
                nn_model.eval()
                v_x, v_y, omega, delta = simulated_data_gen(nn_model, model, avg_vel, dt=dt)
    ...
    if generate_lut:
        ...
        LookupGenerator(racecar_version, save_LUT_name, package_path=package_path).run_generator()
