...
\begin{itemize}
    ...
    \item Simulation evidence under position, velocity, and packet-drop attack cases, together with a closed-loop spacing check during the attack interval using the current IDM/CACC structure through path-longitudinal projection.
\end{itemize}
...
%     \end{bmatrix}.
% \end{equation}}
% 为了避免$\mathrm{wrap}_{\pi}(\cdot)$ 引起的跳变，有如下假设
...
The lemma provides bounded mixing, but bounded mixing alone is not sufficient for convergence because the bicycle model contains prediction dynamics. The accepted trust-induced weights must also provide enough anchor or multi-step information flow to contract the distributed prediction error. Let
\begin{equation}
    M^{(j)}(t)=I_N-\bar{\L}^{(j)}(t).
\end{equation}
...
\begin{equation}
    ...
    \label{eq_M_inf_norm}
\end{equation}
A simple checkable sufficient condition for the one-step contraction used in Theorem~\ref{thm_observer} is that there exists $\underline w_0^{(j)}>0$ such that
\begin{equation}
    w_{i0}^{(j)}(t)\ge \underline w_0^{(j)},\qquad
    L_{x,j}\big(1-\underline w_0^{(j)}\big)<1
    \label{eq_anchor_contraction_check}
\end{equation}
for all hosts $i$ and all admissible times. If direct target-anchor packets are not available at every time step, the same idea can be checked over a finite window by verifying a product contraction,
\begin{equation}
    \left\|
    \prod_{\tau=t}^{t+T_c-1} L_{x,j}M^{(j)}(\tau)
    \right\|_* \le \alpha_c<1,
    \label{eq_window_contraction_check}
\end{equation}
with the factors ordered in time. Equation~\eqref{eq_window_contraction_check} is the natural multi-step counterpart of the one-step condition and is the condition to use for sparse or intermittent anchor availability.
...
\label{ass_graph}
There exists an integer $T_c > 0$ such that for all $t\geq 0$, the union graph $\bigcup_{\tau\in[t,\,t+T_c]}\bar{\G}^{(j)}(\tau)$ contains a spanning tree.
\end{assumption}
Assumption~\ref{ass_graph} means that every node $i \in\V$ is reachable from node ``0'' over each time window $[t,t+T_c]$, ensuring enough connectivity for the observer design~\cite{hao2024eventtriggered}.

This condition is not guaranteed by trust scoring alone; it is a runtime admissibility condition on the accepted trust-induced switches. In implementation, the trust layer must preserve at least one anchored trusted path from node ``0'' to each target over every window $[t,t+T_c]$ and must verify either the one-step contraction check~\eqref{eq_anchor_contraction_check} or the windowed product check~\eqref{eq_window_contraction_check}. If all reliable paths to a target are rejected, the theorem applies only to the reachable part of the platoon, and that target cannot be reconstructed from communication alone.
...
Suppose that condition~\eqref{assume3} holds. If the local-observer error is ISS and Assumption~\ref{assum_distur} holds, then the nonlinear distributed error~\eqref{eq_raw_eDO} is ISS with respect to $e_0^{(j)}$ and $d_{\DO}^{(j)}$. Under the additive attack model, the attacked distributed error is ISS with respect to $e_0^{(j)}$ and $\tilde d_{\DO}^{(j)}$.
\end{lemma}
\begin{proof}
Let $M^{(j)}(t)=I_N-\bar{\L}^{(j)}(t)$. Combining the nonlinear Lipschitz bound~\eqref{eq_nonlinear_lipschitz} with the correction bound~\eqref{eq_nonlinear_consensus_bound} gives
\begin{align*}
\|e_{\DO}^{(j)}(t+1)\|_*
&\le L_{x,j}\|M^{(j)}(t)\|_*\|e_{\DO}^{(j)}(t)\|_*\\
&\quad+L_{x,j}\|w_0^{(j)}(t)\|_*\|e_0^{(j)}(t)\|_*\\
&\quad+\|\tilde d_{\DO}^{(j)}(t)\|_* .
\end{align*}
Condition~\eqref{assume3} makes the homogeneous distributed part a uniform contraction with factor at most $\alpha<1$. The weights are bounded by Lemma~\ref{lem_weight_admissible}, while Lemma~\ref{lem_local_iss} and Assumption~\ref{assum_distur} bound the two inputs. Iterating the scalar inequality above yields a geometric ISS bound. In the nominal case, $d_{\mathrm{att}}^{(j)}=0$ and $\tilde d_{\DO}^{(j)}=d_{\DO}^{(j)}$.
\end{proof}
...
\begin{enumerate}
	...
	\begin{equation} \label{assume3}
        L_{x,j}\left\| I_{N} - \bar{\L}^{(j)}(t) \right\|_* \leq \alpha < 1.
	\end{equation}
    A checkable sufficient condition is~\eqref{eq_anchor_contraction_check} with $\|\cdot\|_*=\|\cdot\|_{\infty}$; for intermittent anchors, the same result can be applied over blocks using~\eqref{eq_window_contraction_check}.
\end{enumerate}
...
\end{theorem}
\begin{proof}
Condition~1 and Assumption~\ref{assum_distur} give the local-observer ISS bound by Lemma~\ref{lem_local_iss}. Condition~2 is satisfied by the proposed trust-weight law according to Lemma~\ref{lem_weight_admissible}, and it prevents each row of the trust-weighted mixing step from amplifying all incoming corrections beyond unit mass. Condition~3 gives the distributed nonlinear contraction used in Lemma~\ref{lem_dist_contraction}. Applying that lemma and taking $\lambda=\max\{\alpha,\lambda_{\ell}\}$ gives the stated bounds after absorbing norm-equivalence constants, bounded Lipschitz constants, and bounded weight gains into $c_0$ and $c_1$. In the nominal case, $d_{\mathrm{att}}^{(j)}=0$ and $\bar{\tilde d}_{\DO}=\bar d_{\DO}$.
\end{proof}

\begin{remark}
The conditions of Theorem~\ref{thm_observer} are nonlinear and local. The compact-region requirement excludes the steering singularity, yaw wrap discontinuities, and unbounded velocities or inputs. In practice, the constants $L_{x,j}$ and $L_{u,j}$ can be conservatively estimated from the Jacobians in~\eqref{eq_bicycle_jacobian}, and the local gain $F_j(t)$ can be checked with the linearized bicycle model. Thus, the proof is stated for the nonlinear observer, while the linearized model remains a computable design tool. For sparse switching graphs, Assumption~\ref{ass_graph} defines the windows over which the product condition~\eqref{eq_window_contraction_check} is verified. Bounded attacks produce bounded estimation degradation, and trust rejection reduces the corresponding input gain by driving the malicious weights to zero.
\end{remark}
...
To make the contribution identifiable, the numerical validation separates the value of trust weighting, local-observer anchoring, rollback, and closed-loop fallback through matched baseline configurations.
...
In the saved-run analysis, attack-window RMSE is computed over $[10,15]$~s for non-attacker vehicle/case/run entries. Trust detection delay is measured from the attack onset at $10$~s to the first time at which the mean trust assigned to the attacker falls below the reporting threshold $\theta_{\mathrm{op}}=0.50$. 
...
Figure~\ref{fig:qcar_limo_scheme} summarizes the experimental flow. The host vehicle stack contains the sensor interface, local observer, vehicle logic, and control loop. Peer vehicles run the same stack and exchange UDP packets containing local estimates and trust reports. The trust-based observer converts consistency checks into adaptive fusion weights and produces a resilient fleet-state estimate for visualization, validation, and controller use. In this platform validation, however, the longitudinal controller is the existing CACC implementation; it does not yet blend CACC with an IDM or ACC fallback. The attack channel can inject corrupted, delayed, or replayed packets so that the no-rollback and rollback cases can be compared under the same vehicle trajectory and communication structure.
...
\begin{figure*}[!t]
    ...
    \begin{tikzpicture}[
        ...
        \draw[flow, userorange] (localobs.east) -- ++(0.44,0) |- ([yshift=-0.16cm]udp.west);
        \node[flowlabel] at (6.66,5.36) {broadcast};
        \draw[flow, userorange] ([yshift=-0.10cm]peers.west) -- ++(-0.52,0) |- ([yshift=0.16cm]localobs.east);
        \node[flowlabel] at (6.66,2.66) {received};
        \draw[flow, userpurple] (udp.east) -- (trusteval.west);
        \draw[flow, userpurple] (attack.east) -- ++(0.58,0) |- (distobs.west);
        ...
        \draw[feedback, userblue] (fleetest.south) -- ++(0,-0.22) -| (closedloop.south);
        \node[flowlabel, text width=2.05cm, align=center] at (8.65,0.47) {fleet estimate to controller};
    \end{tikzpicture}}
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[
        ...
        \node[box, right=0.65cm of long, yshift=-0.42cm] (plant) {
            \textbf{2D bicycle}\\
            $(X,Y,\psi,v)$\\
            state update
        };
        ...
        \draw[arrow, green!50!black] (long.east) -- (plant.west |- long.east);
        \draw[arrow, orange!80!black] (lateral.east) -- (plant.west |- lateral.east);
        \draw[fb] (plant.south) -- ++(0,-0.9cm) -| (context.south);
    \end{tikzpicture}}
    \caption{Compact vehicle-level controller for the QCar/LIMO 2D kinematic model used in the rollback validation. The current platform test applies CACC for $a_i$, while lateral tracking produces $\delta_i$.}
...
Motivated by the residual deviation observed in the CACC-only platform rollback test, this subsection formulates a mixed local/cooperative longitudinal controller. The purpose is to combine the cooperative following benefit of CACC with a local IDM/ACC fallback when the distributed estimates become unreliable. The bicycle-state observer remains in planar coordinates, and the spacing controller uses the road-aligned projection
\begin{align}
    e_{\parallel}(k)&=[\cos\psi_{\mathrm{ref}}(k),\sin\psi_{\mathrm{ref}}(k)]^\top,\\
    s_i(k)&=p_i(k)^\top e_{\parallel}(k),
\end{align}
where $p_i=[X_i,Y_i]^\top$. For each follower $i\ge2$, the controller computes two commands: a local IDM command driven by onboard information and a cooperative CACC command driven by the distributed observer estimates of the predecessors $\mathcal{P}_i=\{1,\ldots,i-1\}$. The final acceleration is obtained by trust-based command fusion, so unreliable V2V estimates affect only the cooperative channel.

The closed-loop validation uses the worst numerical attack condition identified by the RMSE analysis. Let $i^\star$ denote the vehicle with the largest attack-window estimation degradation. The main safety metric is the minimum projected bumper-to-bumper spacing
\begin{equation}
...
The local command is the standard IDM acceleration
\begin{align}
    u_{\mathrm{loc},i}
    &\equiv a_{\text{IDM},i} \notag\\
    &=\alpha\Bigg[
    ...
    \Bigg],
\end{align}
where $s_{i-1,i}$ is the measured/projected bumper spacing to the immediate predecessor, $\Delta v_{i-1,i}=v_{i-1}-\hat{v}_0^{(i)}$, and
\begin{equation}
    s^*(\hat{v}_0^{(i)},\Delta v_{i-1,i})=s_0+T\hat{v}_0^{(i)}+\frac{\hat{v}_0^{(i)}\Delta v_{i-1,i}}{2\sqrt{\alpha\beta}}.
\end{equation}
The parameters $\alpha$, $\beta$, $\delta_{\mathrm{IDM}}$, $v_0$, $s_0$, and $T$ are the maximum acceleration, comfortable deceleration, free-flow exponent, desired velocity, minimum spacing, and time headway. Since $u_{\mathrm{loc},i}$ does not use V2V packets, it provides the fallback when the distributed layer is down-weighted.

The cooperative command uses the distributed estimates
$\hat p_i^{(j)}=[\hat X_i^{(j)},\hat Y_i^{(j)}]^\top$ and $\hat v_i^{(j)}$ of each predecessor $j\in\mathcal{P}_i$, together with the host's local ego estimate
$\hat p_0^{(i)}=[\hat X_0^{(i)},\hat Y_0^{(i)}]^\top$ and $\hat v_0^{(i)}$:
\begin{align}
    u_{\mathrm{coop},i}
...
This validation should therefore be interpreted as a spacing-preservation check during the tested attack interval, not as a complete string-stability proof.
