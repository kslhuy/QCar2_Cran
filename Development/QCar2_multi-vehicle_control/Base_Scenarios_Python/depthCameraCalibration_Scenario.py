...
def setup(qlabs):
    ...
    myCar.spawn_id_degrees(actorNumber=0, location=[0,-1.25,0.5], rotation=[0,0,0], scale=[0.1,0.1,0.1])
    myCar.possess()
    ...
    #generate Wall at appx. 1m from depth camera
    myWall = QLabsBasicShape(qlabs)

    myWall.spawn_degrees(location=[2.2,-1.25,0.01], rotation=[0,0,0], scale=[0.1,1,0.2], configuration=0)
    myWall.set_material_properties(color=[0.1,0.2,0.1], roughness=1.0)

    #generate taller Wall at 2m

    myWall.spawn_degrees(location=[1.2,-1.25,0.01], rotation=[0,0,0], scale=[0.1,1,0.1], configuration=0)
    myWall.set_material_properties(color=[0.5,0.1,0.1], roughness=1.0)
