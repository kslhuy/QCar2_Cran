...
class VehicleLeaderController_RTmodel:
    ...
    def compute_control_w_observer(self, dt=0.1):
        """
        NOT USED IN PROCESS-BASED ARCHITECTURE.
    ...
    def stop_control(self):
        ...
        # Stop QCar immediately - always send zero commands when stopping
        if self.qcar is not None:
            try:
                self.qcar.write(0, 0)
                print(f"Vehicle {self.vehicle_id}: QCar stopped with zero commands")
            except Exception as e:
