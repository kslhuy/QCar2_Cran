...
$cameraScriptPath = Join-Path $platoonFolder "camera_follow.py"
...
if ($initLines -match $successPattern) {
    ...
    # 启动相机跟随脚本
    if (Test-Path $cameraScriptPath) {
        Write-Host "[start_platoon] 等待 2 秒后启动相机跟随..." -ForegroundColor DarkGray
        Start-Sleep -Seconds 2
        
        $cameraCmd = "cd `"$platoonFolder`"; & `"$activateScript`"; python camera_follow.py"
        Write-Host "[start_platoon] 启动相机跟随脚本" -ForegroundColor Cyan
        Start-Process powershell -ArgumentList "-NoExit", "-Command", $cameraCmd
        Write-Host "[start_platoon] 相机跟随脚本已启动！" -ForegroundColor Green
    } else {
        Write-Warning "[start_platoon] 未找到相机跟随脚本: $cameraScriptPath"
    }
    
    Write-Host "`n[start_platoon] 编队启动完成！" -ForegroundColor Green
    Write-Host "[start_platoon] - 4辆车辆已启动 (car-id 0-3)" -ForegroundColor Green
    Write-Host "[start_platoon] - 相机跟随已启动 (跟随车辆0)" -ForegroundColor Green
}
else {
